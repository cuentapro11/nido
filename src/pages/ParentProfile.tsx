...
export default function ParentProfile() {
  ...
  const { children, parentChildId, updateChild } = useApp();
  const { show } = useToast();
  ...
  const upd = (field: keyof Child) => (val: string) => {
  ...
  return (
    <div className="min-h-screen bg-background pb-10">
      ...
      <div className="bg-card">
        <EditRow label="Notas" value={child.notas} onChange={upd('notas')} placeholder="Añadir nota" />
        <EditRow label="Alergias" value={child.alerts.join(', ')} onChange={v => updateChild(child.id, { alerts: v ? v.split(',').map(s => s.trim()) : [] })} placeholder="Añadir alergias" />
        <EditRow label="Medicamento" value={child.medicamentos} onChange={upd('medicamentos')} placeholder="Añadir medicamento" />
      </div>
