...
export default function Configuracion() {
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      <PageHeader title="Administración" subtitle={cuidoInfo.nombre} />
      <div className="px-3.5 space-y-3 pt-1">

        {/* Profile card */}
        <div className="bg-card rounded-2xl shadow-card overflow-hidden">
          <div className="bg-gradient-to-r from-primary to-accent px-4 py-4 flex items-center gap-3">
            <button onClick={() => photoRef.current?.click()} className="relative flex-shrink-0 active:scale-95 transition-transform">
              {profilePhoto ? (
                <img src={profilePhoto} alt="Perfil" className="w-14 h-14 rounded-2xl object-cover border-2 border-primary-foreground/30" />
              ) : (
                <div className="w-14 h-14 bg-primary-foreground/20 rounded-2xl flex items-center justify-center text-2xl">{cuidoInfo.emoji}</div>
              )}
            </button>
            <div className="flex-1 min-w-0">
