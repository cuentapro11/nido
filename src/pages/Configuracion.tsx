...
import {
  ...
  BookOpen, Search, Mail, Lock, ChevronLeft
} from 'lucide-react';

// ─── Slide Screen ─────────────────────────────────────────────────
function SlideScreen({ title, onBack, children }: { title: string; onBack: () => void; children: React.ReactNode }) {
  return (
    <motion.div
    ...
    >
      <div className="flex items-center gap-3 px-4 pt-10 pb-4 border-b border-border bg-card flex-shrink-0">
        <button onClick={onBack} className="w-9 h-9 rounded-xl bg-muted flex items-center justify-center active:scale-90">
          <ChevronLeft size={20} className="text-foreground" />
        </button>
        <h1 className="flex-1 text-base font-extrabold text-foreground">{title}</h1>
      </div>
...
export default function Configuracion() {
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      <PageHeader title="Administración" topSpacing />
      ...
      <AnimatePresence>
        {activePanel === 'centro' && (
          <SlideScreen title="Centro" onBack={() => setActivePanel(null)}>
      ...
      <AnimatePresence>
        {activePanel === 'equipo' && (
          <SlideScreen title="Equipo" onBack={() => setActivePanel(null)}>
            <div className="px-4 py-4">

        {/* Código de invitación para empleados */}
        <div className="mb-4">
          <p className="text-xs font-bold text-muted-foreground mb-2">Código de invitación para empleados</p>
          <div className="flex items-center gap-3 mb-3">
            <div className="flex-1 bg-muted rounded-xl px-4 py-3">
              <span className="font-extrabold text-foreground tracking-widest">{cuidoCode}-EMP</span>
            </div>
            <button onClick={() => { navigator.clipboard.writeText(cuidoCode + '-EMP').catch(()=>{}); show('Código copiado'); }}
          ...
          <p className="text-xs text-muted-foreground mb-4">Comparte este código con tus cuidadores y profesores para que puedan registrarse en NidoApp.</p>
        </div>

        <div className="flex items-center justify-between mb-4">
          <h3 className="text-base font-extrabold text-foreground">Cuidadores</h3>
      ...
      {/* PANEL: Familias */}
      <AnimatePresence>
        {activePanel === 'familias' && (
          <SlideScreen title="Niños y Familias" onBack={() => setActivePanel(null)}>
            <div className="px-4 py-4">
      ...
      <AnimatePresence>
        {activePanel === 'pagos' && (
          <SlideScreen title="Pagos y Suscripción" onBack={() => setActivePanel(null)}>
            <div className="px-4 py-4">
