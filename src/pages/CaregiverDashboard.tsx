...
function NotifPanel({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, zIndex: 50 }}
      ...
      {/* Header */}
      <div className="bg-gradient-to-r from-[hsl(var(--primary))] to-[hsl(var(--accent))] px-5 pt-4 pb-4 flex items-center justify-between">
        <div>
...
export default function CaregiverDashboard() {
  ...
  return (
    <div className="min-h-screen bg-background pb-6" style={{ position: 'relative', overflow: 'hidden' }}>
      ...
      {/* ── HEADER ──────────────────────────────────────────────────── */}
      <div className="bg-gradient-to-r from-[hsl(var(--primary))] to-[hsl(var(--accent))] px-5 pt-4 pb-3">
        <div className="flex items-start justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setShowNotifs(true)}
              className="relative w-9 h-9 bg-primary-foreground/15 rounded-full flex items-center justify-center active:scale-95 transition-transform">
              <Bell size={16} className="text-primary-foreground" />
              {unreadNotifs > 0 && (
                <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-destructive text-destructive-foreground rounded-full text-[9px] font-bold flex items-center justify-center border-2 border-transparent">
                  {unreadNotifs}
                </span>
              )}
            </button>
