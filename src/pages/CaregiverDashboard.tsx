...
export default function CaregiverDashboard() {
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      ...
      {(tab === 'inicio' || tab === 'log') && (
        <div className="bg-[#1E3A8A] px-5 pt-10 pb-3 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center text-lg flex-shrink-0">
              {daycare?.emoji ?? '🐣'}
            </div>
            <div>
              <p className="text-[9px] font-semibold text-white/55 uppercase tracking-widest leading-none mb-0.5">Cuidador/a</p>
              <h1 className="text-base font-extrabold text-white leading-tight">{profile?.name ?? 'Ana García'}</h1>
            </div>
          </div>
