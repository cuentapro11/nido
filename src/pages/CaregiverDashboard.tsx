...
import {
  ...
  User, BookOpen, Settings, Lock, Building2
} from 'lucide-react';
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        {step > 1 && (
          <button onClick={reset} className="text-xs font-bold text-muted-foreground bg-muted px-3 py-1.5 rounded-lg active:scale-95">Reiniciar</button>
        )}
      </div>
      ...
      {step === 1 && (
        <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.25 }}>
          ...
          <div className="grid grid-cols-4 gap-2.5">
            {LOG_OPTS.map(opt => (
              <button key={opt.type} onClick={() => { setLogType(opt.label); setStep(2); }}
      ...
      {step === 2 && (
        <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.25 }}>
          ...
          {present.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-6">No hay niños presentes. Marca la entrada primero.</p>
          ) : (
            <>
        ...
        <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.25 }}>
          ...
          {logPhoto ? (
            <div className="relative mb-4">
              ...
              <button onClick={() => setLogPhoto(null)} className="absolute top-2 right-2 w-7 h-7 bg-foreground/70 rounded-full flex items-center justify-center">
...
// ─── Más (settings) panel ─────────────────────────────────────────────────
function MasPanel({ profile, daycare, onLogout }: { profile: any; daycare: any; onLogout: () => void }) {
  const navigate = useNavigate();

  function SectionLabel({ label }: { label: string }) {
    return <div className="px-1 pt-4 pb-1.5"><p className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">{label}</p></div>;
  }

  function NavItem({ icon: Icon, label, desc, onClick }: { icon: any; label: string; desc?: string; onClick: () => void }) {
    return (
  ...
  return (
    <div className="px-3.5 space-y-3 pb-8">
      {/* Perfil card */}
      <div className="bg-card rounded-2xl shadow-card overflow-hidden">
        <div className="bg-[#1E3A8A] px-4 py-4 flex items-center gap-3">
          <div className="w-16 h-16 rounded-full bg-white/20 flex items-center justify-center text-3xl shadow-soft flex-shrink-0">
            👤
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-white font-extrabold text-base">{profile?.name || 'Cuidador/a'}</p>
            <p className="text-white/60 text-xs">{profile?.email || 'cuidador@cuido.app'}</p>
            <p className="text-white/60 text-xs mt-0.5">{daycare?.name || 'Cuido'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-card rounded-2xl shadow-card overflow-hidden divide-y divide-border">
        ...
        <NavItem icon={User}     label="Nombre y correo"       desc={profile?.name}         onClick={() => {}} />
        <NavItem icon={Lock}     label="Cambiar contraseña"    desc="Actualiza tu acceso"   onClick={() => {}} />
        <NavItem icon={Building2} label="Mi salón"             desc="Salón asignado"        onClick={() => {}} />
      </div>

      {/* Herramientas */}
      <div className="bg-card rounded-2xl shadow-card overflow-hidden divide-y divide-border">
        <SectionLabel label="Herramientas" />
        <NavItem icon={BookOpen}  label="Reporte Diario"   desc="Solo mis niños"          onClick={() => navigate('/reporte-diario')} />
        <NavItem icon={Settings}  label="Configuración"    desc="Notificaciones, idioma"  onClick={() => navigate('/configuracion-general')} />
      </div>

      {/* Cerrar sesión */}
      <button onClick={onLogout}
        className="w-full bg-card rounded-2xl shadow-card py-4 text-center text-destructive font-bold text-sm active:scale-[0.98] transition-transform">
        Cerrar sesión
      </button>
    </div>
  );
...
export default function CaregiverDashboard() {
  ...
  const [tab, setTab]       = useState<'inicio' | 'children' | 'chat' | 'mas'>('inicio');
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="px-4 pt-4">
        ...
        {tab === 'inicio' && (
          <>
            ...
            <div className="flex gap-2 mb-4">
              ...
              <div className="flex-1 bg-card rounded-xl p-3 text-center shadow-card">
                <div className="text-xl font-extrabold text-destructive">{supaChildren.length - present.length}</div>
                <div className="text-[9px] text-muted-foreground font-semibold uppercase tracking-wide mt-0.5">Ausentes</div>
              </div>
              <div className="flex-1 bg-card rounded-xl p-3 text-center shadow-card">
        ...
        {/* MÁS */}
        {tab === 'mas' && <MasPanel profile={profile} daycare={daycare} onLogout={handleLogout} />}
