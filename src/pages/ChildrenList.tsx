...
export default function ChildrenList() {
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      <PageHeader title="Niños" subtitle={`${children.length} niños registrados`} />
            ...
            <div className="px-3.5 space-y-2">
              {groupChildren.map((c, i) => (
                <motion.div key={c.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}>
                  ...
                  >
                    <ChildAvatar child={c} size={48} showStatus />
                    <div className="flex-1 min-w-0">
                      {/* Punto de color junto al nombre */}
                      <div className="flex items-center gap-1.5">
                        <span className={`w-2 h-2 rounded-full flex-shrink-0 ${c.present ? 'bg-[hsl(152,55%,42%)]' : 'bg-destructive'}`} />
