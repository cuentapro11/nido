          ...
          {filteredTl.map((item, i) => (
            ...
            <div className="bg-card rounded-2xl overflow-hidden shadow-card">
              ...
              {item.hasImg ? (
                ...
                <div>
                  {/* Imagen full width */}
                  <div onClick={() => setFullScreenImg(item.photo)} className="w-full aspect-square bg-muted cursor-pointer active:opacity-90 transition-opacity">
                    <img src={item.photo} alt="Activity" className="w-full h-full object-cover" />
                  </div>
                  {/* Caption centrada debajo */}
                  <div className="px-4 pt-3 text-center">
                    <p className="text-[12px] text-gray-400 font-medium mb-0.5">{item.time}</p>
                    {item.desc && <p className="text-[13px] text-gray-600 leading-relaxed">{item.desc}</p>}
                  </div>
                  {/* Acciones abajo */}
                  <div className="flex items-center justify-center gap-6 px-4 py-4 mt-2">
                    <button onClick={() => setLikedItems(prev => ({ ...prev, [i]: !prev[i] }))}
                      className="flex items-center gap-1.5 active:scale-90 transition-transform">
                      <Heart size={22} className={likedItems[i] ? 'text-destructive fill-destructive' : 'text-muted-foreground/60'} strokeWidth={1.5} />
                    </button>
                    <button className="flex items-center gap-1.5 active:scale-90 transition-transform">
                      <Download size={22} className="text-muted-foreground/60" strokeWidth={1.5} />
                    </button>
                  </div>
                </div>
              ) : (
