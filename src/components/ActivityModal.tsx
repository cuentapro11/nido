...
export default function ActivityModal({ open, onClose, preselectedChildId }: Props) {
  ...
  const handlePickOpt = (opt: typeof LOG_OPTS[0]) => {
    ...
    if (preselectedChildId != null) {
      ...
      // Bloquear actividades (no entrada/salida) si el niño no tiene entrada
      if (opt.type !== 'entry' && opt.type !== 'exit' && ch && !ch.present) {
        show(`⚠️ ${ch.name} no tiene entrada registrada`);
        setSelectedOpt(null);
        return;
      }
